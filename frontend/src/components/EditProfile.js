                ...
                <div className={styles.flexboxPro}>
                                ...
                                editCellModal ? (
                                    <>
                                        <div style={{width:"60%", marginRight:"5%"}}>
                                            Cell:  <input style={{width:"170px"}} type="text" value={cell} onChange={(e) => setCell(e.target.value)} />
                                        </div>
                                        <DoneIcon onClick={() => setEditCellModal(false)} sx={{fontSize:"0.9rem",fontWeight:"500", mt:1.5, "&:hover":{cursor:"pointer"} }} />
                                    </>
                                ):(
                                    <>
                                        <div style={{width:"50%", marginRight:"5%"}}>Cell:  {cell}</div>
                                        <EditIcon onClick={() => setEditCellModal(true)} sx={{fontSize:"0.9rem", mt:"5px", "&:hover":{cursor:"pointer"} }} />
                                    </>
                                )
                            }
                    ...
                    <div className={styles.rightFlexPro}>
                            ...
                            awards.map(
                                (award) => (
                                    <div style={{marginLeft:"0px", display:"flex"}} className='optionContainer'>
                                        <label style={{marginLeft:"50px", marginRight:"50px"}} className="ckbox-container-show">{award}
                            ...
                            editAwards && (
                                <div >
                                    ...
                                    <div style={{display:"flex", marginLeft:"50px"}}>
                                        ...
                                        <input style={{"marginLeft":"0%", "width":"50%", borderRadius:"4px"}} type="text" placeholder="Enter Award Name" value={newAward} onChange={(e) => setNewAward(e.target.value)} />
                                        
                                        <div style={{ marginLeft:"2%", marginTop:"22px"}} className={styles.saveBtn} onClick={() => newAwardAdded()}>
                                            Save
                                        </div>

                                        <div style={{ marginLeft:"1%", marginTop:"22px"}} className={styles.cancelBtn} onClick={() => {setEditAwards(false); setNewAward("");}}>
                                            Cancel
                                        </div>
                        ...
                        <div style={{display:"flex", marginTop:"20px"}}>
                            ...
                            {
                                !editHours && (
                                    <div onClick={() => {setEditHours(true); }} className={styles.addNewOption}>
                        ...
                        <hr className='line-psy' style={{width:"90%"}}></hr>
                        {
                            hours.map(
                                (hour) => (
                                    <div style={{marginLeft:"0px", display:"flex"}} className='optionContainer'>
                                        <label style={{marginLeft:"50px", marginRight:"50px"}} className="ckbox-container-show">{hour}
                                            <input type="checkbox" value={hour} disabled/>
                                            <span className="ckbox-checkmark-show"></span>
                                        </label>
                                        <IconButton onClick={() => removeHour(hour)} sx={{mt:"-10px", '&:hover':{backgroundColor: '#a39d9d',}}}> <DeleteIcon sx={{ color:"#f03224"}} /> </IconButton>
                                    </div>
                                )
                            )
                        }
                            ...
                            editHours && (
                                <div >
                                    ...
                                    <div style={{display:"flex", marginLeft:"50px"}}>
                                        ...
                                        <input style={{"marginLeft":"0%", "width":"50%", borderRadius:"4px"}} type="text" placeholder="Enter Time" value={newHour} onChange={(e) => setNewHour(e.target.value)} />
                                        
                                        <div style={{ marginLeft:"2%", marginTop:"22px"}} className={styles.saveBtn} onClick={() => newHourAdded()}>
                                            Save
                                        </div>

                                        <div style={{ marginLeft:"1%", marginTop:"22px"}} className={styles.cancelBtn} onClick={() => {setEditHours(false); setNewHour("");}}>
                                            Cancel
                                        </div>
