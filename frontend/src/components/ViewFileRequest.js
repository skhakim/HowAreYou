...
export function ApproveFileRequest(props) {
    ...
    return (
        <div className='container'>
            ...
            <div className='save' onClick={() => approveFR(props.frID)}>
                 Approve
            </div>

            <div className='cancel' onClick={() => rejectFR(props.frID)}>
                 Decline
            </div>
        </div>
    )
...
export function DownloadFileRequest(props) {
    ...
    const fetchResponses = async () => {
        const res = await fetch('/vfr' + props.frID, {
            method: "GET", headers: {
                'Accept': 'application/vnd.api+json'
                , 'x-access-token': getToken()
            }
